...
- TXT、Markdown 源码和分栏模式继续使用稳定的 CodeMirror；Markdown 预览模式已切换为真正的 Milkdown/Crepe 所见即所得编辑器。两类编辑器均支持中文输入、查找替换、撤销重做、选区处理、行号和 `Ctrl + 鼠标滚轮` 调整字号。
- TXT 与 Markdown 共用同一份原始文本，支持源码、分栏和预览三种 Markdown 编辑方式。
- Markdown “预览”采用 Milkdown/Crepe 单栏即时渲染，支持斜杠菜单、块拖动、选区格式工具栏、链接、任务列表、表格、代码块、公式和图片等 Markdown 能力，最终仍保存为纯 Markdown。
