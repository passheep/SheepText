...
- **文件夹面板**：从本地文件打开文稿时，左上角出现「文件夹」按钮，悬停展开、点击常驻，列出同目录的 TXT / Markdown 文稿，点击即可在当前窗口切换标签。
...
- 在资源管理器里多选多个 TXT / Markdown 后右键「打开方式 → SheepText」，会统一在同一个窗口的不同标签中打开。
...
- 在 Markdown 编辑区按 `Alt + →` 触发；在设置里开启「自动补全」后，打字停顿约 1 秒也会自动触发。
- 建议以灰色文字出现在光标处，按 `→` 或点提示条上的「采用」写入正文，`Esc` 或点关闭按钮丢弃。
- 灰字只存在于独立的装饰层：不写入文稿、不触发自动保存，`Ctrl + Z` 一次即可撤销。
- 输入法组字过程中不触发，避免打断中文输入。

### 用量统计

- 设置中的「用量统计」页按时间范围、模型、类型三个维度筛选。
- 展示总 tokens、输入、输出、缓存命中与未命中、命中率、调用次数与失败次数。
- 按天展示柱状图，明细可在「按日期」与「按模型」之间切换。
...
需求、方案与验收文档保存在开发工作区的 `文档/` 目录（不在本仓库内），包括编辑体验与窗口恢复、本地文件与交互修复、多标签标题栏、标签与交互优化、第四轮体验优化，以及 AI 补全方案调研等。